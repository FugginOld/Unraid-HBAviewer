...
| 026 | Complete diagnostic bundle, with consistent anonymisation | P2 | M | — | EXECUTED on `advisor/026-diagnostic-bundle` (`bb6b872`, `cc54d0c`), unmerged. New `scripts/bundle_support.sh` + `bundle.php` + a Settings card pairing beside Notifications; the button uses `formaction` so the one settings form posts to the endpoint (no nested form, no JS, Unraid still injects its `csrf_token`). Sections 1–5 collected with raw tool output and the parsed JSON side by side, both `/cN/eall/sall` and `/cN/sall`, nothing from `/boot/config` bar the plugin's own cfg, `zip` checked for with a `tar czf` fallback (`zip` was absent here, so the fallback is what ran). Anonymisation replaces **words**, which makes length preservation structural rather than something to remember. Two bugs the tests caught: keying the map case-sensitively gave one PHY two pseudonyms (sysfs writes `sas_address` lower, storcli upper) — exactly this plan's stated failure arriving through a door it did not name; and the plan's 16-char `SERIAL0000000001` trimmed to `00000001` on a real 8-char serial. 45 assertions in `tests/anon_test.sh`, 12 in `tests/bundle_php_test.php`; suite green, no golden moved. **Nothing run against hardware** — the storcli/lsiutil/SMART capture blocks were exercised only on their absent-tool paths |
