...
| 022 | Per-PHY error baseline, with rate shown alongside the raw counter | P1 | M | — | DONE — on `advisor/022-phy-error-baseline` (`2086e91`, `931555c`), not yet merged; **not hardware-verified**. The row previously read NOT WRITTEN, which was stale: the plan was written and then revised on 2026-07-31 (see the revisions table below). **Executed display-only** — the plan's "feed delta/rate into the PHY error floor" scope was deliberately dropped, because `PHYERR_FLOOR` belongs to plan 018 (issue #8, hardware-verified) and the rollup badge to plan 020 (also hardware-verified); wiring a third input would have had two verified fixes fighting over one badge. `storcli_overview.sh` and `health.php` are untouched. Step 1's open choices were settled as: per-controller granularity, `/boot/config/plugins/hbaviewer/phy_baseline.json`, and option 1 on reset detection (invalidate and tell the user, never silently rebase) |
