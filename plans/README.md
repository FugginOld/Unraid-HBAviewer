...
| 026 | Complete diagnostic bundle, with consistent anonymisation | P2 | M | — | **REWRITTEN 2026-08-01** against `fe90641`, in progress. Scope expanded by the maintainer from a support-bundle sketch to a complete diagnostic generator with real anonymisation. The prior version had anonymisation as a `:` placeholder. Key design constraint now documented: **naive per-file redaction is worse than none** — it breaks the SAS-address join between PHY and drive data (the join `drives_join.sh` calls "where the historical drive bugs lived") and reflows storcli's fixed columns, so the bundle can no longer serve as a test fixture. Requires one bundle-wide, length-preserving pseudonym map. Explicit never-collect list covers the Unraid flash GUID, licence key and everything in `/boot/config` bar the plugin's own cfg |
