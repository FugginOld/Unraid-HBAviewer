...
| 021 | Replace hardcoded panel colors with Unraid theme variables | P1 | S | — | DONE — merged to `dev` (`e0259a4`, from `0cbf845`). Chrome tokens bound to `--background-color`/`--shade-bg-color`/`--border-color`; eight hardcoded pastels retired in favour of `--crit-text`/`--good-text`/`--warn-text`. Closes issue #7. **Not hardware-tested.** Making the card surfaces theme-aware invalidated plan 018's band-contrast measurement; **that fallout is resolved by plan 030**, now also merged. The light/dark standard agreed with the maintainer is recorded in the plan's "Outcome" section — read it before adding any new card, tile or tab |
| 030 | Gradient temperature bands, instrument tile, Health gauge | P1 | L | 021 (DONE) | DONE — merged to `dev`, from `643108e`. Maintainer chose decision (A): the Health gauge counts indicators whose state is `ok` and reads `N / total` — no invented score. Five bands are now dark→light gradients (`lsi_temp_gradient`); `lsi_temp_color` survives for the critical chip only; `lsi_temp_stroke` and the dead `temp_color` view key are gone. Both conic-gradient rings replaced by one shared `lsi_gauge_svg()` half-circle arc. Tile treatment keyed on `lsi_tile_is_light()`. Band boundaries, `flex` weights and label percentages untouched; goldens unmoved. **Not hardware-tested.** Two follow-ups noted in the executor report: the plan's `#2ecc71/#f1c40f/#e67e22/#e74c3c/#ff5252` → 0 done-criterion is unsatisfiable as written (those hexes also belong to `lsi_status_color`/`lsi_health_color`/chart series, all out of scope), and `lsi_display_pref()` had to fall back to `dynamix.cfg` because the AJAX endpoints never receive `$display` |
