...
Active plans (still in `plans/`): **022** (executed, awaiting a reboot test),
**024–029** (written, not started), **035**, **037** and **038** (executed,
awaiting a hardware check), **036** (written, not started).
...
| **038** | That a real SAS9207-8i reads **Gen3 (8.0 GT/s)** after the fix. Every fixture in the suite is synthetic, so the tests prove the decode table matches `mpi2_cnfg.h` — not that lsiutil on real hardware emits what we think it does. The `get_hba_health.sh` copy of the table has **no automated coverage at all**. | Needs @jac2424 (issue #9) or another SAS2/lsiutil box: patch `parse/hba.sh` and `get_hba_health.sh` in place, clear `/tmp/lsiutil_dash.json`, and confirm the Overview and dashboard tile read Gen3 against their `lspci` `LnkSta`, and that the Health tab's `host_link` row shows the same speed. **The maintainer's box cannot test this** — it is SAS3/storcli and never reaches this code. While they are there, ask for `hbaviewer.x86_64 -p1 -a 25,2,0,0` to settle the `CurrentPowerMode` follow-up in the plan |
