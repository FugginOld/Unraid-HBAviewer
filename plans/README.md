...
**024–029** (written, not started), **035** and **037** (executed, awaiting a
hardware check), **036** (written, not started).
...
| 035 | Per-controller cards on the two JavaScript-built tabs | P3 | M | 033 (DONE) | **EXECUTED** on `advisor/035-js-tab-cards` (**not pushed, not merged**), from `7db512d`. Completes 033. Performance drops its wrapper card, moves the toolbar to pane level (reusing 033's `.lu-tab-pane > .lu-tab-toolbar` inset) and cards each `.lu-perf-ctl`; `.lu-perf-ctl`'s `margin-bottom` deleted as `.lu-card` supplies it. Firmware/BIOS keeps its danger and array-state banners in their own card and closes it before `#flash-content`; `.lu-fc` loses border/radius/padding/margin/background to `.lu-card` and survives as a hook only. `data-ctl` added to the flash error branch — **which required making `flashChip()` null-safe in the same change**: `flashCard()` can now find a box that has no `data-chip`, and `URLSearchParams` stringifies the resulting `null` to `"null"`, which `flash.php`'s alnum filter accepts and its non-empty guard passes, so a bogus chip would have reached `escapeshellarg`. No flashing or chart logic touched; no golden moved. **Both tabs are JS-built, so the PHP render tests cannot reach them — there is no automated coverage of this change.** Verified instead by executing `perfBuild()` and `flashChip()` under a DOM shim. **HARDWARE CHECK OUTSTANDING** — needs two controllers: two Performance cards with live charts, banners in one card above two flash cards, and the flash controls still targeting the right `data-ctl` |
...
| 037 | Grey out flash Step 3 until the array is stopped | P3 | S | 035 (EXECUTED) | **EXECUTED** on `advisor/037-flash-step3-gate` (**not pushed, not merged**), from `7a2f259` (tip of 035). Firmware/BIOS Step 3 gets `is-locked` plus `disabled` on its checkbox, FLASH field and button while the array runs; Steps 1 and 2 stay live on purpose — Step 1 is read-only and Step 2 only stages an image into the plugin's tools dir, so leaving it open lets a user upload *before* taking the array down, which shortens the outage. **Affordance only** — `flash.php` is byte-identical and `luFlashGo`'s `!flashArrayStopped` alert survives verbatim; deleting this CSS would still leave flashing blocked. Two judgements the plan did not cover: (1) the plan's `.is-locked .lu-flock { opacity: 1 }` is a **no-op** — CSS opacity applies to the whole subtree and a child can never be less transparent than its parent — so the explanation renders as a **sibling above** the locked step, not inside it; (2) **opacity raised 0.45 → 0.6**, because 0.45 measures 2.3:1 (azure) and 2.8:1 (white) against those themes' own card background, below even the 3:1 threshold; 0.6 lands 3.3–6.2:1 across all four themes and still reads as dimmed. `cursor: not-allowed` skipped — `pointer-events: none` means the element is not a hit target, so the rule can never fire without adding a wrapper purely to hold it. **This tab is JS-built, so the PHP render tests cannot reach it — the suite passing proves only that nothing leaked into the renderers.** Verified by executing the real `luFlashInit` source under a DOM shim with the flag both ways. **HARDWARE CHECK OUTSTANDING**, see the checklist below |
| 022 | Per-PHY error baseline, with rate shown alongside the raw counter | P1 | M | — | **EXECUTED** on `advisor/022-phy-error-baseline` (pushed, **not merged**). Baseline JSON store under `/boot/config/plugins/hbaviewer/`, per-controller "Set Baseline" button, delta + errors/hr rendered inside the existing counter cells (no new columns, so no golden moved). 29 unit tests. Endpoint lives in `phy_baseline.php` with its own POST gate rather than in `ajax_info.php`, which declares itself read-only. **Rollup integration deliberately dropped** — `PHYERR_FLOOR` belongs to plan 018/issue #8 and the badge to plan 020, both hardware-verified; a third rate source would fight them. Display only. **HARDWARE CHECK OUTSTANDING — needs a reboot**, see the checklist below |
...
| **022** | Reset-by-reboot detection. The `/boot` baseline outlives the counters it measured, so without this the tab shows a large negative delta. Only ever run against synthetic uptimes. | Apply the branch, press Set Baseline, `reboot`. Expect "baseline reset by reboot", **not** a negative number. `modprobe -r mpt3sas && modprobe mpt3sas` exercises the other signal (counter decrease) without rebooting — array must be stopped |
