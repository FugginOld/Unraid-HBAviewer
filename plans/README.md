...
| 023 | Notify through Unraid's `notify` on health status transitions | P1 | M | — | **EXECUTED** on `advisor/023-notification-hooks` (pushed, **not merged**). `ENABLE_NOTIFY` off by default, cron every 10 min, one notification per status *change* and never a repeat while it persists; state keyed on `board_name@pci_location` so two identical cards cannot mask each other; an errored read is skipped rather than read as a false all-clear. Plan 001's "does not send notifications" sentence corrected. 26 unit tests. **Transition logic verified on hardware 2026-08-01** (first sighting silent, transition fires once, repeats silent, recovery fires). **The cron path is NOT verified** — see the checklist below |
