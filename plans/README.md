...
| 031 | Show the thermal indicator the Health gauge already counts | P2 | S | 030 (DONE) | **EXECUTED** on `advisor/031-health-thermal-row` (not pushed, not merged). One-line fix: `thermal` added as the first entry of `ajax_info.php`'s indicator-row list, and `controller` relabelled `Read Health` to match `hbaviewer.php`'s header sentence. `health.php` untouched, no golden moved, `hbaviewer.php` needed no edit. 18 new assertions in `tests/ajax_render_test.php`; the load-bearing pair is gauge-denominator == rows rendered and gauge-numerator == green rows, verified to fail against the pre-fix line. Covers a no-sensor card (`thermal` unknown, renders `—`, not counted ok). **Not yet verified on hardware** |
| 032 | Health rows — status dots and Tabler icons | P3 | S | 031 (EXECUTED, unmerged) | **EXECUTED** on `advisor/032-health-row-icons`, branched from 031 (not pushed, not merged). `.lu-ind-bar` (30×9 bar) → `.lu-ind-dot` (8px round) **keeping the two-layer gradient fill and the `--gd`/`--gl` inline props from `lsi_health_gradient()`** — the maintainer's mockup fills the dots flat, but three of those five colours fail the 3:1 floor on the white/azure card (`ok` 2.74, `watch` 1.50, `warning` 2.15), which is the defect plan 030 fixed; the CSS comment now records that the gradient survived the shape change and why. Five Tabler symbols (MIT, paths byte-identical to `plans/assets/hba-health-card.html`) emitted **once** in `hbaviewer.php`'s `#lu-wrap`, never from the per-poll `ajax_info.php`; ids `lu-i-` prefixed against webGui collisions. Icons 15px against the row's 12.5px text. `link_integrity`→`lu-i-link` and `host_link`→`lu-i-hostlink` need the one inline map. The mockup's `alert-triangle` was left out (belongs to a status pill this plan does not add). 6 new assertions in `tests/ajax_render_test.php`, incl. every `<use href>` resolving to a symbol actually defined in the shell. No golden moved. **Not yet verified on hardware** |
...
| 034 | Lay the Settings page out in two columns | P3 | S | none | **EXECUTED** on `advisor/034-two-column-settings`, branched from 032 (not pushed, not merged). One file: `settings.php`. A `.lu-s-grid` wrapper **inside** `<form>` holds the three `.lu-s-card` sections; the Advanced/Flashing card carries an explicit `.lu-s-span` class for `grid-column: 1 / -1` (a class, not `:last-child`, so the span reads as deliberate — the plan's maintenance note calls that out). The Save button and the post-save Monitor link sit outside the wrapper, so they stay in normal flow below the grid instead of becoming grid items. **No breakpoint was invented**: none existed in either file (`grep @media` → nothing), so the grid reuses the `repeat(auto-fit, minmax(…, 1fr))` idiom already at `hbaviewer.php:303` — `minmax(min(360px, 100%), 1fr)` collapses to one column below ~784px viewport and cannot overflow on a phone. Required judgement call not in the plan: `#lu-settings-wrap`'s `max-width` had to go **580px → 1000px**, because at 580px two tracks are 258px each against a 180px `.lu-s-label` basis and the grid would never reach two columns anyway; 1000px also caps it at two tracks (a third needs 1112px). No `name=` attribute, input, label or default touched (`grep -o 'name="[^"]*"'` byte-identical); danger colours and `config.php` untouched. `bash tests/run.sh` all pass, no golden moved, `php -l` clean. **Not yet verified on hardware** |
