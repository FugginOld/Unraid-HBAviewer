...
**024–029** (written, not started), **035–036** (written, not started).
...
| 035 | Per-controller cards on the two JavaScript-built tabs | P3 | M | 033 (DONE) | **WRITTEN** 2026-08-01 against `4338f68`, not started. Completes 033, which missed Performance and Firmware/BIOS because both build their per-controller boxes in JavaScript inside `hbaviewer.php` rather than in `ajax_info.php`'s PHP renderers — a survey of the renderers could not see them. The flash tab differs from the other five: its danger and array-state banners are page-level and stay in their own card above the controller cards. Also fixes the flash error branch omitting `data-ctl`, which makes `flashCard(i)` unable to find an errored controller's box today |
