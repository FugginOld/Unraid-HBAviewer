...
| 023 | Notify through Unraid's `notify` on health status transitions | P1 | M | — | **EXECUTED 2026-08-01**, branch `advisor/023-notification-hooks` (`4445ed9` logic+store+tests, `bafe21d` toggle, `0f60046` cron) — **not merged, not hardware-verified**. Suite green, no golden moved, `plans/README.md`'s own "Direction 1" note now implemented. Both pieces shipped: `notify.php` (pure `notify_transitions()`, identity = `board_name@pci_location` not array index, `/boot` last-notified store) and a `/etc/cron.d/hbaviewer` entry installed *and removed* from the `.plg`'s two hooks. **Step 1's `STATUS` discrepancy resolved: this index's "failed drives, PHY error counters and pre-P20 firmware" describes only the storcli backend, and overstates it.** `parse/storcli_overview.sh` does fold drive state (`Failed/Offln/Missing/UBad/Foreign`→alert, `Rbld/Copyback`→warn) and a 100-count PHY floor into `status`; `parse/hba.sh` (lsiutil/SAS2) is **temperature-band only**, so SAS2 boxes get thermal-only alerting; pre-P20 firmware is `fw_old`, a separate field, in **neither** rollup. Left alone — 018/020 territory. Three items need a real box: `notify`'s flag set on 7.2+, that `/etc/cron.d` + `update_cron` is the right hook, and one clean install/uninstall cycle (the suite cannot exercise either) |
