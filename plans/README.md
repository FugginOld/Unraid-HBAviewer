...
| 037 | Grey out flash Steps 2 and 3 until the array is stopped | P3 | S | **035** | **WRITTEN** 2026-08-02 against `7a2f259`, in progress. Today `flashArrayStopped` is used in exactly one place — an `alert()` at `hbaviewer.php:695` fired *after* the user has ticked the box, typed FLASH and clicked. This surfaces the precondition up front. **Step 1 stays active deliberately**: it is read-only and the only useful action while the array is up. The disable is an **affordance, not a control** — `flash.php`'s `flash_array_stopped()` guard and the line-695 alert both stay, and a reviewer should confirm that deleting the CSS entirely would still leave flashing blocked |
