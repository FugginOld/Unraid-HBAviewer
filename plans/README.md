...
in `plans/`** — 029 parked on a reboot the maintainer has to do, 058 not
...
| **058** | **not started**, written 2026-08-08 | nobody. The half of the original firmware-index design that did NOT ship: a mirrored `firmware/` tree seeded from 45Drives, a generated `manifest.json`, and per-image download links pinned to a release tag. Split out when the *verdict* half was specced and built as `docs/superpowers/specs/2026-08-08-known-firmware-verdict-design.md` — that spec supersedes sections 1-6 and 9 of 058's README, and the README says so at the top. **What survives only here** is §7's update path, §8's repository constraints (the 45Drives mirror publishes no checksums; GitHub refuses the 132 MB LSA zip; rehosting Broadcom binaries wants a provenance and takedown note; committed binaries bloat history permanently) and §10's open questions. Carries `build-firmware-manifest.py`, whose `SAS3324`/`SAS3316` chip typos were corrected when the verdict work proved the real parts are `SAS3224`/`SAS3216`. Two draft files were deleted rather than moved: `FirmwareIndex.php`, superseded by `firmware_index.php` with three false-positive paths closed that the draft still had, and `known-firmware.json`, which shipped corrected as the plugin's `data/known-firmware.json` |
