...
**024–029** (written, not started), **035** (executed, awaiting a hardware
check), **036** (written, not started).
...
| 037 | Grey out flash Step 3 until the array is stopped | P3 | S | **035** | **WRITTEN** 2026-08-02 against `7a2f259`, in progress. Today `flashArrayStopped` is used in exactly one place — an `alert()` at `hbaviewer.php:695` fired *after* the user has ticked the box, typed FLASH and clicked. This surfaces the precondition up front. **Steps 1 and 2 stay active deliberately**: Step 1 is read-only, and Step 2 only uploads an image to the plugin's own tools directory — keeping it open lets a user stage the image before taking the array down, which shortens the outage. Step 3 is where hardware gets written, so it is the one that locks. The disable is an **affordance, not a control** — `flash.php`'s `flash_array_stopped()` guard and the line-695 alert both stay, and a reviewer should confirm that deleting the CSS entirely would still leave flashing blocked |
