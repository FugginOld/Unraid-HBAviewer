...
| **P2-D** | Stale-data indicator on every polled tab | **Narrow version DONE; the broad one deliberately not built.** The blocking-read fix made the tile serve stale values *on purpose*, so "stale" became two things and one indicator firing on both would train people to ignore it. What shipped instead needs no threshold: `cached_read()` reports `stalled` when its own lock outlived `lock_ttl` — the previous producer never came back — and the Overview says the last read did not finish rather than showing "reading …" forever. Called *stalled*, not *died*: 120s is a long time but a very slow controller could still be working. Showing data AGE everywhere remains open and still needs a maintainer's threshold. |
