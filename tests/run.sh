...
if command -v node >/dev/null 2>&1; then
    ...
    MSYS_NO_PATHCONV=1 docker run --rm \
        -v "$(cd .. && { pwd -W 2>/dev/null || pwd; }):/app" -w /app/tests \
        node:20-alpine node diagnose_js_test.js; diagnose_js_fail=$?
else
    echo "SKIP  diagnose_view.js runtime tests (no node, no docker)"; diagnose_js_fail=0
fi
...
echo
if [ $fail -eq 0 ] && [ $nl_fail -eq 0 ] && [ $flash_fail -eq 0 ] && [ $flash_js_fail -eq 0 ] && [ $diagnose_js_fail -eq 0 ] && [ $baymap_js_fail -eq 0 ] && [ $sort_js_fail -eq 0 ] && [ $anon_fail -eq 0 ] && [ $read_smart_fail -eq 0 ] && [ $health_sh_fail -eq 0 ] && [ $drives_sysfs_fail -eq 0 ] && [ $topology_fail -eq 0 ] && [ $multiport_fail -eq 0 ] && [ $storcli_count_fail -eq 0 ] && [ $locate_sh_fail -eq 0 ] && [ $phys_json_fail -eq 0 ] && [ $bundle_coverage_fail -eq 0 ] && [ $collect_smart_fail -eq 0 ] && [ $drive_triage_fail -eq 0 ] && [ $get_defects_fail -eq 0 ] && [ $php_fail -eq 0 ]; then
