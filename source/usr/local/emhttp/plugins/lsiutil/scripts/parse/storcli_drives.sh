...
function emit(){
    if (!first) printf ","
    first=0
    printf "{\"slot\":\"%s\",\"model\":\"%s\",\"serial\":\"%s\",\"state\":\"%s\",\"sas_address\":\"%s\",\"size\":\"%s\",\"link\":\"%s\",\"firmware\":\"%s\"}", \
        eid"/"slot, model, sn, state, wwn, size, link, fw
}
BEGIN { first=1; have=0; printf "{\"drives\":[" }
/^Drive \/c[0-9]+\/e[0-9]+\/s[0-9]+ :[ \t]*$/ {
    if (have) emit()
    match($0, /e([0-9]+)\/s([0-9]+)/, a); eid=a[1]; slot=a[2]
    model=""; sn=""; state=""; wwn=""; size=""; link=""; fw=""; have=1
    next
}
