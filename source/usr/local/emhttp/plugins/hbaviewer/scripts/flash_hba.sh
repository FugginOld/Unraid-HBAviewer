...
if [ -z "$tool" ]; then
    case "$gen" in
        sas2|sas3) die "${gen}flash is required to flash $chip, and it is not installed. Broadcom does not permit bundling it. Put it in /boot/config/plugins/hbaviewer/tools/${gen}flash (or upload it under Step 1), and make sure it is executable. Cards on the 9400/9500 generation use storcli instead and need nothing extra." 4 ;;
